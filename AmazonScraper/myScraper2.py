...
recherche = input('Veuillez entrer vos mots clés: ')
url = "https://www.amazon.fr/s?k={}".format('+'.join(recherche.split()))
soup = get_data_page(url)
items_url = get_items_urls_from_all_pages(url)
table = generate_my_items_table(items_url[:10])

books = pd.DataFrame(table, columns=['Titres', 'Format Prix', 'Nombre de pages', 'Dimensions Format Broché', 'Dimensions Format Poche', 'Dimensions Format Relié', 'Evaluations (sur 5)', 'Nombre d\'évaluations', 'Liens' ])
books.to_csv('aya.csv')


# Test
'''
url = 'https://www.amazon.fr/R%C3%A9gime-C%C3%A9tog%C3%A8ne-efficacement-programme-d%C3%A9butants-ebook/dp/B087YTMPF3/ref=tmm_kin_swatch_0?_encoding=UTF8&qid=1618574884&sr=8-317-spons'
soup = get_data_page(url)
links = get_links_for_each_item_format(soup)
print(get_item_broché_dimension(soup, links))
'''